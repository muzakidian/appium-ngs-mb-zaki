...
import { MESSAGE_PYTHON_MISSING, NAME_MIKE, NAME_MKDOCS, NAME_MKDOCS_YML, NAME_PACKAGE_JSON, NAME_PYTHON, } from './constants.js';
import { DocutilsError } from './error.js';
import { getLogger } from './logger.js';
import { findPackageRoot, mergeDefaultsDeep, readPackage, } from './utils/index.js';
 ...
 */
export const stringifyYaml = (value) => YAML.stringify(value, undefined, { indent: 2 });
/**
 * Pretty-stringifies a JSON value
 * @param value Something to stringify
 * @returns JSON string
 */
export const stringifyJson = (value) => JSON.stringify(value, undefined, 2);
/**
 * Reads a YAML file, parses it and caches the result
 */
const readYaml = util.memoize(async (filepath) => YAML.parse(await fs.readFile(filepath, 'utf8'), {
    prettyErrors: false,
    logLevel: 'silent',
}));
/**
 ...
 * @returns
 */
export async function findInPkgDir(filename, cwd = process.cwd()) {
    try {
...
async function _readPkgJson(cwd, normalize) {
    ...
    try {
        ...
        throw new DocutilsError(`Could not find a ${NAME_PACKAGE_JSON} near ${cwd}; please create it before using this utility`);
    }
 ...
 */
export const readJson = util.memoize(async (filepath) => JSON.parse(await fs.readFile(filepath, 'utf8')));
/**
 * Writes contents to a file. Any JSON objects are stringified
 * @param filepath - Path to file
 * @param content - File contents
 */
export function writeFileString(filepath, content) {
 ...
 */
const cachedWhich = util.memoize(fs.which);
/**
 ...
 */
export const isMkDocsInstalled = util.memoize(async () => {
    // see if it's in PATH
 ...
 */
export const findPython = util.memoize(async () => (await whichPython3()) ?? (await whichPython()));
/**
 * Check if a path to Python exists, otherwise raise DocutilsError
 */
export async function requirePython(pythonPath) {
 ...
 */
export const readMkDocsYml = util.memoize(async (filepath, cwd = process.cwd()) => {
    ...
    if (mkDocsYml.INHERIT) {
        let inheritPath = path.resolve(path.dirname(filepath), mkDocsYml.INHERIT);
        while (inheritPath) {
