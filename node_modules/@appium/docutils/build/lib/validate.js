...
import { MESSAGE_PYTHON_MISSING, NAME_BIN, NAME_ERR_ENOENT, NAME_MKDOCS, NAME_MKDOCS_YML, NAME_PIP, NAME_PYTHON, NAME_REQUIREMENTS_TXT, REQUIREMENTS_TXT_PATH, } from './constants.js';
...
export class DocutilsValidator extends EventEmitter {
    ...
    async validateMkDocsConfig() {
        ...
        try {
            ...
            if (err.code === NAME_ERR_ENOENT) {
                return this.fail(`Could not find ${NAME_MKDOCS_YML} at ${mkDocsYmlPath}. Use --mkdocs-yml to specify a different path.`);
            }
    ...
    async validatePythonDeps() {
        ...
        if (missingPackages.length) {
            msgParts.push(`The following required ${util.pluralize('package', missingPackages.length)} could not be found:\n${missingPackages.map((p) => `- ${p.name} @ ${p.version}`).join('\n')}`);
        }
        if (invalidVersionPackages.length) {
            msgParts.push(`The following required ${util.pluralize('package', invalidVersionPackages.length)} are installed, but at the wrong version:\n${invalidVersionPackages
