import { DEFAULT_COMPARE_OPTIONS, DEFAULT_FORMAT_STRING, DEFAULT_PIXELMATCH_OPTIONS, DEFAULT_SHADOW, DEFAULT_TABBABLE_OPTIONS, FULL_PAGE_SCROLL_TIMEOUT, STORYBOOK_FORMAT_STRING, } from './constants.js';
...
const { ignoreAlpha: _ignoreAlpha, ignoreAntialiasing: _ignoreAntialiasing, ignoreColors: _ignoreColors, ignoreLess: _ignoreLess, ignoreNothing: _ignoreNothing, ...SHARED_COMPARE_DEFAULTS } = DEFAULT_COMPARE_OPTIONS;
/**
...
export class CompareOptionsConflictError extends Error {
    constructor(context, presentIgnoreKeys) {
        super('Cannot combine ignore* options with pixelmatch options.\n' +
 ...
 */
export function hasIgnoreOptionKeys(options) {
 ...
 */
export function hasPixelmatchOptions(options) {
 ...
 */
export function getCompareMode(options) {
 ...
 */
export function stripPixelmatchOptions(options) {
 ...
 */
function describeCompareMode(options, mode) {
    if (mode === 'pixelmatch') {
 ...
 */
export function warnOnMethodCompareModeOverride(wic, method, commandName) {
    const serviceMode = getCompareMode(wic);
    const methodMode = getCompareMode(method);
    log.warn('Method compare options override service compare mode for %s.\n' +
        '  Service: %s\n' +
        '  Method:  %s', commandName, describeCompareMode(wic, serviceMode), describeCompareMode(method, methodMode));
}
/**
 * Merges service and method compare options; method wins and strips the opposing compare mode.
 */
export function resolveEffectiveCompareOptions(wic, method, commandName) {
    assertExclusiveCompareMode(method, commandName);
    const wicMode = getCompareMode(wic);
    const methodMode = hasPixelmatchOptions(method)
        ? 'pixelmatch'
        : hasIgnoreOptionKeys(method)
            ? 'preset'
            : null;
    const merged = { ...wic, ...method };
    if (methodMode === 'pixelmatch') {
        if (wicMode === 'preset') {
            warnOnMethodCompareModeOverride(wic, method, commandName);
        }
        return {
            ...stripIgnoreOptionKeys(merged),
            pixelmatch: {
                ...wic.pixelmatch,
                ...method.pixelmatch,
            },
    ...
    if (methodMode === 'preset') {
        ...
        return stripPixelmatchOptions(merged);
    }
    return merged;
}
/**
 ...
 */
export function assertExclusiveCompareMode(options, context) {
 ...
 */
export function resolvePixelmatchOptions(userOptions) {
    return {
 ...
 */
function buildCompareOptions(options) {
...
export const IGNORE_PRESET_ORDER = ['alpha', 'antialiasing', 'colors', 'less', 'nothing'];
const IGNORE_PRESET_OPTION_NAMES = {
    alpha: 'ignoreAlpha',
 ...
 */
export function resolveActiveIgnorePreset(ignoreList) {
 ...
 */
export function resolveComparePreset(ignoreList) {
...
export function prepareIgnoreOptions(imageCompareOptions) {
    return IGNORE_PRESET_ORDER.filter((option) => Object.keys(imageCompareOptions).find((key) => key.toLowerCase().includes(option) && imageCompareOptions[key]));
}
/**
 ...
 */
export function warnOnMultipleIgnorePresets(ignoreList) {
    ...
    log.warn('Multiple ignore* compare options are enabled (%s). Only one preset is applied per comparison.\n' +
        '  Active: %s (last-wins order: ignoreAlpha → ignoreAntialiasing → ignoreColors → ignoreLess → ignoreNothing).', enabledOptions, activeOption);
}
