 ...
 */
function isHighlightedPixel(output, offset, diffColor, aaColor, diffColorAlt) {
    const r = output[offset];
    const g = output[offset + 1];
    const b = output[offset + 2];
    const matchesColor = (color) => r === color[0] && g === color[1] && b === color[2];
...
export default async function compareImages(image1, image2, options) {
    ...
    const resolvedPixelmatch = pixelmatchSettings ?? {
