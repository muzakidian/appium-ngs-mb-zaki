...
describe('pixelmatch adapter - compareImages', () => {
    ...
    describe('ignore option mapping', () => {
        ...
        ])('passes preset settings for ignore: %s', async (ignore, expected) => {
            pixelmatchFn.mockImplementation(() => 0);
            await compareImages(Buffer.from('img1'), Buffer.from('img2'), { ignore });
            expect(pixelmatchFn).toHaveBeenCalledWith(expect.anything(), expect.anything(), expect.anything(), expect.any(Number), expect.any(Number), expect.objectContaining(expected));
        });
