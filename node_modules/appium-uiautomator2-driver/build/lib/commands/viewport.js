 ...
 */
export async function mobileViewportElementRect(elementId) {
    if (!this.isWebContext()) {
        throw new errors.InvalidContextError('The current context must be a web view in order to translate an element rect into native screen coordinates');
    }
    ...
    if (clamped.width === 0 || clamped.height === 0) {
        throw new errors.ElementNotInteractableError(`The element is not visible: its translated rectangle does not overlap the WebView's on-screen bounds`);
    }
    return clamped;
}
/**
 ...
 */
export async function enrichWebviewsMappingWithRects(driver, mapping) {
    let hasCdpRect = false;
    for (const m of mapping) {
        for (const page of m.pages ?? []) {
            const rect = parseWebviewRectFromPage(page);
            if (rect) {
                m.rect = rect;
                hasCdpRect = true;
                break;
    ...
    if (!hasCdpRect && mapping.length === 1) {
        try {
            mapping[0].rect = await getNativeWebViewRectFromViewHierarchy(driver);
        }
        catch {
 ...
 */
function parseWebviewRectFromPage(page) {
 ...
 */
async function getWebviewRectFromCdp(driver) {
    let mapping;
    try {
 ...
 */
async function getNativeWebViewRectFromViewHierarchy(driver) {
    ...
    const settled = await Promise.allSettled(webViewElements.map((el) => {
        const elementId = util.unwrapElement(el);
        return driver.uiautomator2.jwproxy.command(`/element/${elementId}/rect`, 'GET');
    }));
    const rects = settled
    ...
    if (!rects.length) {
        throw new errors.NoSuchElementError('Could not determine the bounds of any visible native WebView element on screen');
    }
    if (rects.length > 1) {
        throw new errors.UnknownError('Found multiple visible native WebView elements on screen and could not determine which one hosts the ' +
 ...
 */
async function getNativeWebViewRect(driver) {
 ...
 */
async function getWebviewGeometryContext(driver) {
    const chromedriver = driver.chromedriver;
