...
export class MjpegFrameParser extends Transform {
    ...
    startFrame(length, chunk, start, end) {
...
export class MJpegStream extends Writable {
    ...
    async start(serverTimeout = DEFAULT_SERVER_TIMEOUT_MS) {
        this.stop();
        ...
        const onClose = () => {
            ...
            this.registerStartFailure?.(new Error(`The connection to the MJPEG stream at ${url} has been closed before any frame was received`));
        };
        let timeoutId;
        const startPromise = new Promise((resolve, reject) => {
            this.registerStartSuccess = resolve;
            this.registerStartFailure = reject;
            timeoutId = setTimeout(() => reject(new Error(`Waited ${serverTimeout}ms but the MJPEG server never sent any images`)), serverTimeout);
        });
        this.responseStream
            .once('close', onClose)
            .on('error', onErr)
